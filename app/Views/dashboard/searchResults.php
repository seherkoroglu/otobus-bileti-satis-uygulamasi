      ...
      <?php  foreach ( $seyahatler as $seyahat) { ?>
        <h5 class="card-title
                ...
                <form action="<?= base_url('bilet/biletBul'); ?>" method="post">
                    ...
                    <p>Sefer Numarası: <?=$seyahat['tripID']; ?> | Sefer Tipi: <?= $seyahat['tripType']; ?></p>
                    <p>Kalkış Terminali: <?= $seyahat['departureTerminal']; ?> (<?= $seyahat['departureCity']; ?>) | Varış Terminali <?= $seyahat['arrivalTerminal']; ?> (<?= $seyahat['arrivalCity']; ?>)</p>
                    <p>Kalkış Zamanı <?= $seyahat['departureTime']; ?> | Varış Zamanı <?= $seyahat['arrivalTime']; ?> </p>
                    <button type="submit" class="btn btn-primary">Sefer Seç</button>
                    <br><br>
                </form>
        <?php } ?>
        <?php endif; ?>
